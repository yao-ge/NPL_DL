...
void packet_info_t::set_masks(packet_value_t **ref) {
    ...
    for (i = 0; i < 2; i++) {
        if (ref[i] != NULL) {
            masks[i]->deep_copy(ref[i]);
        }
    }
}

void packet_info_t::set_masks(int index, packet_value_t *ref) {
    masks[index]->deep_copy(ref);
}
...
int packet_info_t::compare(packet_info_t *ref, packet_info_t *ref_mask) {
    ...
    for (i = 0; i < 2; i++) {
        if (ref_mask == NULL) {
            if (masks[i]->compare(ref->masks[i], NULL) == 1) {
                mismatch = 1;
            }
        } else {
            if (masks[i]->compare(ref->masks[i], ref_mask->masks[i]) == 1) {
                mismatch = 1;
            }
        }
    }

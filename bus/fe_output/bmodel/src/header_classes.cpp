...
int pss_t::extract_fields(_8bits *pkt, _32bits *hdr_ptr, _32bits *pkt_len) {
    uint byte_ptr;
    uint i;

    start_byte = *hdr_ptr;
    if (*pkt_len < (start_byte+1024)) {
        // Remaining packet bytes are less than fixed header size
        *hdr_ptr = *pkt_len;
        return -1;
    }
    ...
    for (i = 0; i < 1024; i++) {
        if (i != 1023) {
            values |= (((_8192bits)(pkt[*hdr_ptr + byte_ptr + i]) << (8 * (1024 - i - 1))));
        } else {
            values |= (((_8192bits)(pkt[*hdr_ptr + byte_ptr + i]) >> 0));
        }
    }
...
void pss_t::pack() {
    ...
    for (i = 0; i < 1024; i++) {
        if (i != 1023) {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((values) >> (8 * (1024 - i - 1))));
        } else {
            hdr_bytes[byte_ptr + i] |= (_8bits)(((values) << 0));
        }
    }
    update_header_location();
...
void pss_t::print() {
#ifndef NO_CONSOLE_OUTPUT
    bmi_log->normal("pss_t->values = 0x" + values.to_string(SC_HEX_US, false));
    bmi_log->normal("pss_t->_PRESENT = 0x" + _PRESENT.to_string(SC_HEX_US, false));
    bmi_log->normal("pss_t->_PRESENT_CUR = 0x" + _PRESENT_CUR.to_string(SC_HEX_US, false));
#endif
}

void pss_t::deep_copy(pss_t *ref) {
    uint i;
...
void payload_grp_t::set_pss(pss_t **ref) {
    uint i;
    for (i = 0; i < 2; i++) {
        pss[i]->deep_copy(ref[i]);
    }
}

void payload_grp_t::set_pss(int index, pss_t *ref) {
    pss[index]->deep_copy(ref);
}

payload_grp_t::payload_grp_t() {
    uint i;
    for (i = 0; i < 2; i++) {
        pss[i] = new pss_t;
    }
...
void payload_grp_t::set_resetval() {
    uint i;
    for (i = 0; i < 2; i++) {
        pss[i]->set_resetval();
    }
...
void payload_grp_t::deep_copy(payload_grp_t *ref) {
    uint i;
    for (i = 0; i < 2; i++) {
        pss[i]->deep_copy(ref->pss[i]);
    }
}
...
void ingress_pkt_t::set_payload(payload_grp_t *ref) {
    uint i;
    payload->deep_copy(ref);
}
...
void ingress_pkt_t::set_resetval() {
    ...
    payload->pss[0]->hdr_id = 3;
    hdr_ptr_array[3] = payload->pss[0];
    payload->pss[1]->hdr_id = 4;
    hdr_ptr_array[4] = payload->pss[1];
}
...
void egress_pkt_t::set_payload(payload_grp_t *ref) {
    uint i;
    payload->deep_copy(ref);
}
...
void egress_pkt_t::set_resetval() {
    ...
    l2_hdr->l2->hdr_id = 5;
    hdr_ptr_array[0] = l2_hdr->l2;
    l3_hdr->ipv4->hdr_id = 6;
    hdr_ptr_array[1] = l3_hdr->ipv4;
    l4_hdr->udp->hdr_id = 7;
    hdr_ptr_array[2] = l4_hdr->udp;
    payload->pss[0]->hdr_id = 8;
    hdr_ptr_array[3] = payload->pss[0];
    payload->pss[1]->hdr_id = 9;
    hdr_ptr_array[4] = payload->pss[1];
}

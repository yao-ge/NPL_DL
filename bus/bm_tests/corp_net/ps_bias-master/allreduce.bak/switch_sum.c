...
int32_t padding_header_to_buf(char *buf, int32_t *pkt_len, int32_t data_len)
{
	...
	*pkt_len += udp_len;
	...
	header->pkt_len = htonl(2048 + ether_len + ipv4_len + udp_len + 14);
...
#endif
	//printf("recv: job id:%d, max worker:%d, worker id:%d, sequence:%d\n", 
	//		key, size, rank, block);
	padding_header_to_buf(buff, &pkt_len, data_len);
...
int send_reset(int key, int size, int rank, int block)
{
//	printf("This is a UDP client\n");
	int len = sizeof(sock_addr);
	char buff[BUF_SIZE] = {0};
	...
	int block_len = BYTES * BLOCK_SIZE + sizeof(short) * 7;
#endif    
	//printf("reset: job id:%d, max worker:%d, worker id:%d, sequence:%d\n", 
	//		key, size, rank, block);
	padding_header_to_buf(buff, &pkt_len, 0);
    short key_s =  htons((short)key);    
	memcpy( buff+ pkt_len, (char*)&key_s, 2 );
    ...
    short exp_s = htons((short)0);
    memcpy( buff+ pkt_len+8, (char*)&exp_s, 2);

    short bias_s = htons((short)0);
    memcpy( buff+ pkt_len+10, (char*)&bias_s, 2);

    short bias_exp_s = htons((short)0);
    memcpy( buff+ pkt_len+12, (char*)&bias_exp_s, 2);

	block_len += pkt_len;

    printf("sent block len:%d\n", block_len);
    //print_chars(buff, block_len);
	//int n = sendto(sock, buff, block_len, 0, (struct sockaddr *)&sock_addr, sizeof(sock_addr));
	int n = send(sock, buff, block_len, 0);
    if (n < 0)
